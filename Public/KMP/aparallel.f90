...
contains
    ...
    subroutine check_parallel()
        ...
        if (p_sqrt*p_sqrt .ne. mpisize) then
        ...
        my_down = mod((my_row - 1) + p_sqrt,p_sqrt)*p_sqrt+my_col
        my_up   = mod(my_row + 1,p_sqrt)*p_sqrt+my_col
        my_left = my_row*p_sqrt + mod((my_col - 1) + p_sqrt,p_sqrt)
        my_right= my_row*p_sqrt + mod(my_col + 1,p_sqrt)
        write(*,*) "mpirank=",mpirank,"my_up=",my_up,"my_down=",my_down,"my_right=",my_right,"my_left=",my_left
    end subroutine
